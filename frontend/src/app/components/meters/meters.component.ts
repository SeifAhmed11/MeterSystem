...
import { MeterService, MeterDto } from '../../services/meter.service';
    ...
    <div class="meters-container">
      <div class="page-header">
        <div class="brand">
          ...
          <div>
            ...
            <p>عرض العدادات والبحث بالـ Serial</p>
          </div>
        </div>
      ...
      <div class="grid">
        <div class="card-simple">
          <h3 class="card-title">بحث بالـ Serial</h3>
          <form [formGroup]="queryForm" class="form">
            ...
            <div class="actions">
              <button type="button" class="btn-search" (click)="search()" [disabled]="loading || !queryForm.value.serial">
        ...
        <div class="card-simple">
          <h3 class="card-title">بيانات العداد</h3>
            ...
            <table>
              ...
              <tbody>
                <tr *ngFor="let m of meters">
                  <td>{{m.serial}}</td>
                  <td>{{m.type || '—'}}</td>
                  <td>{{m.installedDate ? (m.installedDate | date:'short') : '—'}}</td>
                  <td>{{m.totalRecharges ?? '—'}}</td>
                  <td>{{m.totalConsumptions ?? '—'}}</td>
                </tr>
                <tr *ngIf="!loading && meters.length === 0">
...
export class MetersComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private meterService: MeterService) {
    this.queryForm = this.fb.group({ serial: [''] });
  }
  ...
  loadAll(): void {
    this.loading = true;
    this.meterService.list().subscribe({
      next: (res) => {
        const raw = (res as any)?.data ?? res ?? [];
        this.meters = Array.isArray(raw) ? raw.map(this.mapRow) : [this.mapRow(raw)];
        this.loading = false;
      },
  ...
  search(): void {
    ...
    if (!serial) { this.loadAll(); return; }
    this.loading = true;
    this.meterService.getBySerial(serial).subscribe({
      next: (res) => { const data: any = (res as any)?.data ?? res; this.meters = data ? [this.mapRow(data)] : []; this.loading = false; },
      error: () => { this.loading = false; this.meters = []; }
    });
  }

  private mapRow = (item: any) => {
    // شكل ريسبونس العدادات المباشر
    return {
      id: item?.id,
      serial: item?.serial,
      type: item?.type,
      installedDate: item?.installedDate,
      totalRecharges: item?.totalRecharges,
      totalConsumptions: item?.totalConsumptions
    };
  }
